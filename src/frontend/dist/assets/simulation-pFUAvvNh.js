...
import { c as createLucideIcon, E as commonjsGlobal, F as getDefaultExportFromCjs, r as reactExports, a as clsx, R as React2, G as invariant } from "./index-B6KDdV9-.js";
import { h as addProcessesToStore, e as getProcessStore, u as updateProcessStore } from "./api-DS5_eO34.js";
/**
...
class AdaptiveOverrideMonitor {
  constructor(getCurrentThroughput, onOverride) {
    __publicField(this, "intervalId", null);
    __publicField(this, "consecutiveLowCount", 0);
    __publicField(this, "isOverrideActive", false);
    __publicField(this, "onOverride");
    __publicField(this, "getCurrentThroughput");
  ...
  check() {
    ...
    if (isBelowThreshold) {
      ...
      if (this.consecutiveLowCount >= CONSECUTIVE_REQUIRED && !this.isOverrideActive) {
...
function greedyScore(process) {
  const remaining = Math.max(process.remaining_time, 1e-3);
  return 0.6 * process.priority + 0.4 * (1 / remaining);
}
function greedySort(queue) {
  return [...queue].sort((a2, b2) => {
    const scoreDiff = greedyScore(b2) - greedyScore(a2);
    if (Math.abs(scoreDiff) < 1e-4) return a2.arrival_time - b2.arrival_time;
    return scoreDiff;
  });
}
function resolvedPriority(greedyScoreVal, mlImpactScore) {
  return 0.7 * greedyScoreVal + 0.3 * mlImpactScore;
}
class RoundRobinScheduler {
  constructor(quantum = 10) {
    __publicField(this, "policy", "round_robin");
    __publicField(this, "quantum");
    this.quantum = quantum;
  }
  schedule(queue) {
    const active = queue.filter((p2) => p2.status !== "completed");
    const completed = queue.filter((p2) => p2.status === "completed");
    const processed = active.map((p2) => ({
      ...p2,
      remaining_time: Math.max(0, p2.remaining_time - this.quantum),
      status: p2.remaining_time <= this.quantum ? "completed" : "running"
    }));
...
class SJFScheduler {
  constructor() {
    __publicField(this, "policy", "sjf");
  }
  schedule(queue) {
    const active = queue.filter((p2) => p2.status !== "completed").sort((a2, b2) => a2.burst_time - b2.burst_time);
    const completed = queue.filter((p2) => p2.status === "completed");
    const processed = active.map((p2, idx) => ({
      ...p2,
      status: idx === 0 ? "running" : "waiting",
      remaining_time: idx === 0 ? Math.max(0, p2.remaining_time - p2.burst_time) : p2.remaining_time
    }));
    return [
      ...processed.map((p2) => ({
        ...p2,
        status: p2.remaining_time <= 0 ? "completed" : p2.status
      })),
      ...completed
    ];
  }
...
class PriorityScheduler {
  constructor() {
    __publicField(this, "policy", "priority");
  }
  schedule(queue) {
    const active = greedySort(queue.filter((p2) => p2.status !== "completed"));
    const completed = queue.filter((p2) => p2.status === "completed");
    const quantum = 15;
    const processed = active.map((p2, idx) => {
      if (idx === 0) {
        const remaining = Math.max(0, p2.remaining_time - quantum);
        return {
          ...p2,
...
function generatePoissonProcess(lambda, currentTime) {
  ...
  return {
    pid: Date.now() + Math.floor(Math.random() * 1e4),
...
class SimulationEngine {
  constructor(onTick) {
    __publicField(this, "intervalId", null);
    __publicField(this, "tickCount", 0);
    __publicField(this, "ganttEntries", []);
    __publicField(this, "recentLogs", []);
    __publicField(this, "targetCount", 100);
    __publicField(this, "onTick", null);
  ...
  tick() {
    ...
    if (currentCount < this.targetCount) {
      const newProcesses = Array.from(
        ...
        () => generatePoissonProcess(5, now2)
      );
      if (newProcesses.length > 0) addProcessesToStore(newProcesses);
    }
    const policy = this.tickCount % 6 < 2 ? "priority" : this.tickCount % 6 < 4 ? "sjf" : "round_robin";
    ...
    if (currentQueue.length > 0) {
      ...
      const running = updated.filter((p2) => p2.status === "running");
...
export {
  ...
  getLinearRegression as p,
  Cell as q,
  generateCategoricalChart as r,
  formatAxisMap as s,
  AdaptiveOverrideMonitor as t,
  uniqueId as u,
  SimulationEngine as v,
  Square as w,
  CartesianGrid as x,
  Legend as y,
  greedyScore as z
